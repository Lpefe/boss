...
class ARPModal extends React.Component {
    ...
    delete = (record) => {
        this.props.dispatch({
            type: "mi0102Info/delete_ap_static_ip",
            payload: {
    ...
    render() {
        const ModalOptions = {
            ...
            submitType: this.state.editId ? "mi0102Info/update_ap_static_ip" : "mi0102Info/create_ap_static_ip",
            onCancel: this.closeAddModal,
            bodyHeight: 220,
            extraUpdatePayload: this.state.editId ? {
                device_id: this.props.id,
                ap_lan_id: this.props.cta_lan_id,
                id: this.state.editId
            } : {device_id: this.props.id, ap_lan_id: this.props.cta_lan_id,},
            initPayload: {},
            InputItems: [{
                type: "Input",
                labelName: "@静态IP",
        ...
        return <Modal width="900px" maskClosable={false} visible={this.props.visible} title="@ARP静态绑定"
            ...
            <div>
                <Card className="card">
                    <HeaderBar hasAdd={true} addAlias="@添加" add={this.handelModalShow} hasDelete={true}
                               selectedKeys={this.state.selectedRowKeys} delete={this.deleteAll}/>
                    <Table bordered size="middle" rowKey={record => record.id} pagination={false}
                           rowClassName="normal" rowSelection={rowSelection} columns={Columns}
                           dataSource={this.props.mi0102Info.apStatic}/>
                </Card>
